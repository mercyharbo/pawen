...
export async function submitSummitRegistration(
  ...
  try {
    ...
    console.error("submitSummitRegistration unexpected error", error);
    return {
      status: "error",
      message: "We could not submit the registration right now. Please try again.",
    };
  }
}
...
export async function submitExhibitionBooth(
  ...
  try {
    ...
    console.error("submitExhibitionBooth unexpected error", error);
    return {
      status: "error",
      message: "We could not submit the booth request right now. Please try again.",
    };
  }
}
  ...
  try {
    ...
    return await submitJotform({
      ...
      fieldValues: {
        ...
        "submission[27][]": speakerConfirmations.availability,
        "submission[28][]": speakerConfirmations.terms,
        "submission[29][]": speakerConfirmations.consent,
      },
      files: [
        {
    ...
    console.error("submitApplyToSpeak unexpected error", error);
    return {
      status: "error",
